    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <form action="{{ route('employee.update', $employee->id) }}" method="POST" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                    <!-- Name -->
                    <div class="mt-4">
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ $employee->name }}" required autocomplete="name" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" value="{{ $employee->email }}" required autocomplete="username" />
                    ...
                    <div class="mt-4">
                        ...
                        <select id="group_id" name="group_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            ...
                            @foreach($groups as $group)
                                <option {{ $group->id === $employee->group_id ? 'selected' : '' }} value="{{ $group->id }}">{{ $group->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <input id="manager" class="block mt-1" type="checkbox" name="manager" {{ $employee->manager ? 'checked' : '' }} autocomplete="manager" />
